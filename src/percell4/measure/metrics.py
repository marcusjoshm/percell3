"""Built-in measurement metrics and metric registry for PerCell 4.

All metrics are NaN-safe, using numpy's nan* functions so that derived
FOVs with NaN pixels are handled gracefully.
"""
...
class MetricRegistry:
    ...
    def __init__(self) -> None:
        self._metrics: dict[str, MetricFunction] = dict(METRIC_FUNCTIONS)
    ...
    def register(self, name: str, func: MetricFunction) -> None:
        ...
        Args:
            ...
            func: Callable with signature (image_crop, roi_mask) -> float.
    ...
    def compute(self, name: str, image: np.ndarray, mask: np.ndarray) -> float:
        """Compute a named metric for a single ROI.

        Args:
            name: Registered metric name.
            image: Channel image crop (2D array).
            mask: ROI binary mask crop (2D bool array, same shape as image).
