...
Ported from percell3 with UUID-based IDs. Core computational logic unchanged.
"""
...
class ParticleBGResult:
    ...
    Attributes:
        ...
        roi_id: Database ROI ID (bytes UUID) this particle belongs to.
...
def estimate_background_gaussian(
    ...
    Delegates to :func:`~percell4.plugins.peak_detection.find_gaussian_peaks`
...
def compute_background_ring(
    ...
    Args:
        ...
        exclusion_mask: Optional boolean 2D mask to exclude.
...
def process_particles_for_roi(
    ...
    """Process all particles in an ROI, computing per-particle background subtraction.

    For each particle in the ROI:
    ...
    Args:
        roi_id: Database ROI ID (bytes UUID).
    ...
    for label_val in unique_labels:
        ...
        if ring_pixels == 0:
            logger.debug(
                "ROI %r, particle %d: zero ring pixels after exclusion, skipping",
                roi_id, label_val,
            )
