...
class LocalBGSubtractionPlugin(AnalysisPlugin):
    ...
    def run(
        ...
        # Validate that at least one threshold exists for the particle channel
        ...
        for fov_idx, fov_id in enumerate(fov_ids):
            ...
            # Resolve per-FOV segmentation and threshold IDs
            seg_list = [s for s in store.get_segmentations(seg_type="cellular")
                        if s.source_fov_id == fov_id]
            seg_run_id = seg_list[0].id if seg_list else None
            ...
            if seg_run_id is None or thr_run_id is None:
                warnings.append(f"Skipped FOV {fov_id}: missing seg or threshold")
                continue
