...
def _lazy_import_roifile():  # type: ignore[no-untyped-def]
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install percell3[imagej]"
        ) from None
...
def rois_to_labels(
    ...
    (lines, points, etc.) are silently skipped and counted in the returned
    metadata.
    ...
    Returns:
        ...
        with ``roi_count``, ``skipped_count``, and ``skipped_types``.

    Raises:
        FileNotFoundError: If *zip_path* does not exist.
        ValueError: If the file contains no usable area ROIs.
    """
    roiread, ROI_TYPE = _lazy_import_roifile()
    ...
    label_id = 0
    skipped = 0
    skipped_type_names: set[str] = set()

    for roi in rois:
        if roi.roitype not in area_types:
            skipped += 1
            skipped_type_names.add(str(roi.roitype))
            continue
        ...
        if len(rr) > 0:
            ...
            # Polygon produced no pixels (fully outside bounds)
            label_id -= 1
            skipped += 1
