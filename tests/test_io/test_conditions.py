...
class TestDetectConditions:
    def test_multi_condition_with_s_suffix(self):
        ...
        result = detect_conditions(regions)
        ...
        assert result.region_name_map["A549_Control_s00"] == "s00"
        assert result.region_name_map["A549_Control_s01"] == "s01"

    def test_multi_condition_with_numeric_suffix(self):
        regions = ["ctrl_1", "ctrl_2", "treated_1", "treated_2"]
        result = detect_conditions(regions)
        assert result is not None
    ...
    def test_multi_condition_with_fov_suffix(self):
        regions = ["wt_fov0", "wt_fov1", "ko_fov0", "ko_fov1"]
        result = detect_conditions(regions)
        assert result is not None
    ...
    def test_single_prefix_returns_none(self):
        regions = ["control_s00", "control_s01", "control_s02"]
        result = detect_conditions(regions)
