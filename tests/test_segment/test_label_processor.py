...
class TestExtractCells:
    ...
    def test_with_pixel_size(self, processor: LabelProcessor) -> None:
        ...
        cells = processor.extract_cells(
            labels, region_id=1, segmentation_id=1, pixel_size_um=0.65
        )
    ...
    def test_region_and_segmentation_ids(self, processor: LabelProcessor) -> None:
        """CellRecords should carry the provided region_id and segmentation_id."""
        labels = np.zeros((100, 100), dtype=np.int32)
        labels[10:20, 10:20] = 1
        cells = processor.extract_cells(
            labels, region_id=42, segmentation_id=7, pixel_size_um=1.0
        )
        assert cells[0].region_id == 42
