"""Tests for Measurer — per-region measurement engine."""
...
class TestMeasureRegion:
    """Tests for Measurer.measure_region()."""

    def test_measure_single_channel(self, measure_experiment: ExperimentStore):
        """Measure GFP on region_1: should produce measurements for 2 cells x 7 metrics."""
        measurer = Measurer()
    ...
    def test_no_cells_returns_zero(self, tmp_path):
        """Region with no cells should return 0 measurements."""
        ...
        measurer = Measurer()
        count = measurer.measure_region(
            store, region="region_1", condition="control", channels=["GFP"],
        )
        assert count == 0
        store.close()
    ...
    def test_custom_metric_registry(self, single_region_experiment: ExperimentStore):
        ...
        df = single_region_experiment.get_measurements(metrics=["always_42"])
...
class TestMeasureCells:
    ...
    def test_returns_records_without_db_write(self, measure_experiment: ExperimentStore):
        ...
        # Get cell IDs
        cells_df = measure_experiment.get_cells(condition="control", region="region_1")
